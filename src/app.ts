import { Game, Phase, dailyGame } from "./game.js";
import { dailyPuzzleNumber } from "./seed.js";

const game: Game = dailyGame();

// ---------------- VIEW ----------------
function showView(phase: Phase) {
  game.phase = phase;

  (document.getElementById("start-view") as HTMLElement).hidden = phase !== "start";
  (document.getElementById("game-view") as HTMLElement).hidden = phase !== "playing";
  (document.getElementById("summary-view") as HTMLElement).hidden = phase !== "summary";
}
...
function initBoard() {
  ...
  for (let r = 0; r < 3; r++) {
    for (let c = 0; c < 3; c++) {
      const el = document.getElementById(`cell-${r}-${c}`) as HTMLDivElement;
      el.textContent = "";
...
function selectCell(r: number, c: number) {
  ...
  document.querySelectorAll(".cell.selected").forEach((node) =>
...
function getUsedDigits(): Set<number> {
...
function renderNumberPad() {
  const pad = document.getElementById("number-pad") as HTMLDivElement;
  pad.innerHTML = "";
...
function renderMistakes() {
  const cont = document.getElementById("mistakes") as HTMLDivElement;
  if (!cont) { 
    return
  };
...
// ---------------- FEEDBACK ----------------
function flashWrongCell(r: number, c: number) {
  game.mistakes++;
  renderMistakes();
...
function flashCorrectCell(r: number, c: number) {
  const el = document.getElementById(`cell-${r}-${c}`) as HTMLDivElement;
...
function finishGame(won: boolean) {
  ...
  (document.getElementById("summary-stats") as HTMLElement).textContent = game.summary();
...
function setupHelpPopover() {
  ...
  const toggle = (show?: boolean) => {
    if (typeof show === "boolean") popover.hidden = !show;
  ...
  document.addEventListener("click", (e) => {
    ...
    if (!helpBtn.contains(target) && !popover.contains(target)) toggle(false);
  });
}

async function copySummaryToClipboard() {
  const text = document.getElementById("summary-stats")?.textContent ?? "";
