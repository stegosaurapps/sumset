...
function pickClues(rand: () => number): CellPos[] {
  ...
  for (const cell of shuffled) {
    ...
    if (usedRows.has(cell.r)) continue;
    if (usedCols.has(cell.c)) continue;
    chosen.push(cell);
    usedRows.add(cell.r);
    usedCols.add(cell.c);
  }
 ...
 */
export function generatePuzzleFromSeed(seed: number): Puzzle {
  ...
  while (true) {
    ...
    colSums = [0, 1, 2].map(
      (c) => solution![0][c] + solution![1][c] + solution![2][c]
    );
